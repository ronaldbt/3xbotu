...
class EthScannerService:
    ...
    def __init__(self):
        ...
        # Inicializar executor específico para ETH 4h
        from app.services.auto_trading_eth4h_executor import AutoTradingEth4hExecutor
        self.executor = AutoTradingEth4hExecutor()
    ...
    async def _check_current_state(self) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"Error verificando estado ETH (intento {attempt + 1}/{max_retries}): {e}")
    ...
    async def _handle_searching_buy_state(self, df: pd.DataFrame, current_price: float):
        ...
        try:
            ...
            if signals:
                ...
                self._add_log(
                    "INFO",
                    f"🔍 Búsqueda de compra - No se detectaron patrones U | Precio ETH: ${current_price:,.2f}",
    ...
    async def _handle_monitoring_sell_state(self, current_price: float):
        ...
        try:
            self._add_log(
                "INFO",
                f"📊 Monitoreo de ventas - Verificando condiciones de salida | Precio ETH: ${current_price:,.2f}",
                current_price=current_price
            )
    ...
    async def _scan_cycle(self):
        ...
        try:
            ...
            self._add_log(
                f"{state_emoji} Escaneo 4h - Estado: {current_state} | Velas: {len(df)} | Precio ETH: ${current_price:,.2f}",
                "INFO",
            ...
            logger.error(f"Error en ciclo de escaneo ETH 4h: {e}")
    ...
    def _get_current_eth_price(self) -> float:
        ...
        try:
            import requests
            response = requests.get("https://api.binance.com/api/v3/ticker/price", params={"symbol": "ETHUSDT"}, timeout=5)
            response.raise_for_status()
            data = response.json()
