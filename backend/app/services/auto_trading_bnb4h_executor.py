    ...
    async def _execute_buy_for_api_key(self, db: Session, api_key: TradingApiKey, signal: Dict):
        ...
        try:
            ...
            if available_margin < allocated_usdt:
                ...
                logger.warning(f"⚠️ [Bnb4hExecutor] Con ${allocated_usdt:.2f} USDT de margen, se comprarán ${exposure_usdt_expected:.2f} USDT de exposición ({leverage}x leverage)")
            ...
            logger.info(f"✅ [Bnb4hExecutor] Balance suficiente: ${available_margin:.2f} >= ${allocated_usdt:.2f} USDT")
            
            # Calcular exposición con leverage configurado
            entry_price = signal['entry_price']
            exposure_usdt = float(allocated_usdt) * float(leverage)
            logger.info(f"🎯 [Bnb4hExecutor] Margen: ${allocated_usdt:.2f} USDT → Exposición: ${exposure_usdt:.2f} USDT ({leverage}x leverage)")
        ...
        try:
            ...
            if order_data['type'] == 'MARKET':
                if 'quoteOrderQty' in order_data:
                    ...
                    logger.info(f"💰 [Bnb4hExecutor] Calculando quantity:")
                    ...
                    if quantity <= 0:
                        logger.error(f"❌ [Bnb4hExecutor] Quantity calculado inválido: {quantity} (exposure=${exposure_usdt:.2f}, price=${price:.2f})")
                        return {'success': False, 'msg': f'Quantity inválido: {quantity}', 'code': 'INVALID_QUANTITY'}
                    
                    logger.info(f"   📊 Quantity calculado: {quantity:.8f} BNB")
